...
import {
  ...
  Tooltip,
  Legend,
  Filler, // <-- 1. IMPORTAR EL PLUGIN 'Filler'
  ChartData,
  TooltipItem,
  ChartOptions
} from 'chart.js';
...
const MortalityChart: React.FC = () => {
  ...
  const processedData = useMemo(() => {
    ...
    const lineBarPieData: ChartData<'bar' | 'line' | 'pie'> = {
      ...
      datasets: [{
          ...
          borderWidth: 1,
          tension: 0.3,
          fill: false, // Default a false, se activa solo para 'line'
      }],
    };
    
    if (chartType === 'line') {
        lineBarPieData.datasets[0].backgroundColor = 'rgba(139, 92, 246, 0.5)';
        lineBarPieData.datasets[0].borderColor = 'rgba(139, 92, 246, 1)';
        (lineBarPieData.datasets[0] as ChartData<'line'>['datasets'][number]).fill = true;
    }
  ...
  // --- Opciones y Renderizado de Gráficos (sin cambios) ---
  const renderChart = () => {
    const commonPlugins = {
        title: { display: false },
        tooltip: {
    ...
    switch (chartType) {
      ...
      case 'pie': {
        const options: ChartOptions<'pie'> = {
          responsive: true, maintainAspectRatio: false,
          plugins: { ...commonPlugins, legend: { display: true, position: 'top' }, tooltip: { ...commonPlugins.tooltip, callbacks: {
      ...
      case 'scatter': {
        const options: ChartOptions<'scatter'> = {
            responsive: true, maintainAspectRatio: false,
            plugins: { ...commonPlugins, legend: { display: true }, tooltip: { ...commonPlugins.tooltip, callbacks: {
  ...
  } else if (isLoading && !processedData.labels.length) {
  ...
  return (
    <div className="flex h-full flex-col rounded-lg bg-white p-4 shadow-md">
      <div className="flex flex-shrink-0 items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-700">Mortalidad</h2>
        <ChartTypeSelector />
      </div>
