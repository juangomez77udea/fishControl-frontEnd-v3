...
// 1. USAR EL STORE Y LOS TIPOS DE INSUMO QUE YA FUNCIONAN
...
const FoodSupply: React.FC = () => {
  ...
  // 2. CONECTARSE AL STORE EXISTENTE 'useInsumoStore'
  ...
  useEffect(() => {
    // Solo llama a fetchInsumos si la lista en el store está vacía.
    // Si ya se cargaron en la página de "Gestión de Insumos", se reutilizan.
    if (insumos.length === 0) {
      fetchInsumos();
    }
  }, [fetchInsumos, insumos.length]);

  // 4. LÓGICA DE FILTRADO (PRIMERO POR TIPO 'FOOD', LUEGO POR TEXTO)
  ...
  // --- Lógica de Paginación (Scroll Infinito) ---
  useEffect(() => {
    setPage(1);
        ...
        <tbody>
          {displayedSupplies.map(supply => (
            <tr key={supply.id} className="border-t border-gray-200">
              {/* 5. USAR LAS PROPIEDADES DEL TIPO 'Insumo' */}
              ...
              <td className="py-3 text-right text-sm text-gray-600">
                {/* Asumimos que totalWeightKg ya está en el tipo Insumo */}
  ...
  return (
    <div className="flex h-full flex-col rounded-lg bg-white p-4 shadow-md">
      <div className="flex flex-shrink-0 items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-700">Insumos de Alimento en Stock</h2>
